...
def getAveragePercentCoverage(chromosome):
	...
	#coverageLen = sum(CoverageNumber)
	for k in range(coverageLen):
		for i in range(len(chromosome)):
			#print str(i) + ' : ' + str(len(chromosome))
			#print str(k) + ' : ' + str(len(CoverageList))
			if CoverageList[chromosome[i]][k] == '1':
				firstCoveredSum += ((i+1) * CoverageNumber[k])
				break
...
def Genetic(test_list,test_cov,cov_index):
	global TestNumber
...
if __name__ == '__main__':
	...
	for subject in subject_list:
		subject_path = root_path + subject + '/testmethod/dynamic/state/'
		...
		st = time.time()
		ge = Genetic(testlist,coveragelist,coveragenumber)
		ss = getMaxAveragePercentageCoverage(ge)
		l = len(ss)
                ss = ss[randint(0,l-1)]
                #print ss
                result_list = copy.deepcopy(ss)
                for test_index in range(len(result_list)):
			result_list[test_index] = testlist[ss[test_index]]
