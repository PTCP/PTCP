...
if __name__ == "__main__":
    path = '../../subjects/'
    subjects = readFile(path + 'uselist-tosem-add')
    ...
    gn = [4]
    ts = [1.5]
    tosem_path = 'testmethod/' + str(sys.argv[1])
    gran = str(sys.argv[2])
    result = tree()
            ...
            for titem in ts:
                ...
                for i in range(len(apps)):
                    ...
                    if app !='random':
                        ...
                        rt = []
                        for j in range(50):
                            temp = eval(readFile(path + subject + '/testmethod/dynamic/state/' + str(titem) + 'avg-new/evaluate/' + str(gitem) + '/apfdc_total/' + app + str(j) + '.txt')[0])
                            rt.append(temp)
                        tb[i] = sum(rt)/(len(rt)*1.0)
                        apfdc[i].append(sum(rt)/(len(rt)*1.0))
                        tmax = max(tmax,sum(rt)/(len(rt)*1.0))
                        tempf = eval(readFile(path + subject + '/testmethod/dynamic/state/' + str(titem) + 'avg-new/evaluate/' + str(gitem) + '/dectedtime/firsttime/' + app)[0]) 
                        tempa = eval(readFile(path + subject + '/testmethod/dynamic/state/' + str(titem) + 'avg-new/evaluate/' + str(gitem) + '/dectedtime/averagetime/' + app)[0])
                        ft[i].append(tempf)
                        at[i].append(tempa)
                        result[subject][gitem][titem][app]['ft'] = tempf
                        result[subject][gitem][titem][app]['at'] = tempa
    ...
    if os.path.exists(os.getcwd() + '/data_%s_%s'%(str(sys.argv[1]),gran)) == False:
        os.makedirs(os.getcwd() + '/data_%s_%s'%(str(sys.argv[1]),gran))

    metrics = ['apfdc','ft','at'] 
    for metric in metrics:
        f = open('data_%s_%s/%s_all.csv'%(str(sys.argv[1]),gran,metric),'w')
        for app in app_list:
            f.write('%s,'%app)
    ...
    for metric in metrics:
        for gitem in gn:
            f = open('data_%s_%s/%s_all_%s.csv'%(str(sys.argv[1]),gran,metric,gitem),'w')
            for app in app_list:
                f.write('%s,'%app)
            f.write('subject,gn,ts\n')
            for subject in subjects:
    ...
    for metric in metrics:
        for titem in ts:
            f = open('data_%s_%s/%s_all_%s.csv'%(str(sys.argv[1]),gran,metric,titem),'w')
            for app in app_list:
                f.write('%s,'%app)
            f.write('subject,gn,ts\n')
            for subject in subjects:
