...
if __name__ =='__main__':
    ...
    for subject_item in subject_list:
        ...
        for group_index in group_factor:
            ...
            tcp_filelist = ['genetic_withouttime.txt','greedytotal_withouttime.txt','greedyadditional_withouttime.txt','arp_withouttime.txt','genetic_withtime.txt','greedytotal_withtime.txt','greedyadditional_withtime.txt','arp_withtime.txt','random']
            ...
            for tcp_file in tcp_filelist:
                ...
                #    continue
                    ...
                    st = time.time()
                    test = readTCPfile(subject_path + str(gl) + 'avg-new/group' + group_index + '/' + tcp_file)
                    test_index = TestName_Index(test)
                    ss = getAPFDC(test_index)
                    writeFile(subject_path + str(gl) + 'avg-new/evaluate/'+ tt_index + '/',tcp_file,ss)
                    print tcp_file + ' is completed... ' + str(time.time()-st)
                else:
                    for i in tqdm(range(50)):
                        test = readTCPfile(subject_path + str(gl) + 'avg-new/group' + group_index + '/random/random' + str(i) + '.txt')
                        test_index = TestName_Index(test)
                        ss = getAPFDC(test_index)
                        writeFile(subject_path + str(gl) + 'avg-new/evaluate/'+ tt_index + '/','random' + str(i) + '.txt',ss)
