...
def ARP(test_list,cov_list,time_list,group_number,tl_number):
        #length = len(cov_list)
        ...
        for i in range(length):
                for j in range(i,length):
                        temp_distance = getJaccardDistance(cov_list[i],cov_list[j],CoverageNumber)
        ...
        while len(selected) < length:
                ...
                grouped_index = getGroupIndex(grouped,grouped_cov,grouped_time,cov_list[candidate[MaxIndex]],time_list[candidate[MaxIndex]])
...
if __name__ == '__main__':
        ...
        for subject in subject_list:
                ...
                st = time.time()
                arp,tc = ARP(testlist,coveragelist,timelist,g_n,tl_n)
                prioritize_time = time.time() - st
                if countnumber(arp) != len(testlist):
                    print 'not equal !' + str(countnumber(arp)) + ' : ' + str(len(testlist))
                f = open(subject_path + str(tl_n) + 'avg-new/group'+str(g_n)+'/arp_withouttime.txt','w')
                for group_item in arp:
                        for test_item in group_item:
                                f.write(str(test_item) + '\t')
                        f.write('\n')
                f.close()
                f_time = open(subject_path + str(tl_n) + 'avg-new/group'+str(g_n)+'/timearp_withouttime','w')
                f_time.write(str(prioritize_time))
                f_time.close()
                f_tolerate = open(subject_path + str(tl_n) + 'avg-new/group'+str(g_n)+'/toleratearp_withouttime','w')
